...
public class ContactDAO {
	...
	public ContactDAO(Connection db) {
		this.db = db;
	}
	...
	public Contact recordContact(Contact contact) {
		 try {
	            ...
	            for(int i = 0; i < contact.getAddresses().size(); i++) {
	                ...
	                addressStatement.setString(8, currentAddress.getZipCode());
	                addressStatement.setString(9, currentAddress.getCountry());

	                currentAddress.setId(addressId);

	                addressStatement.addBatch();

	                if(i+1 % 1000 == 0 || i+1 == contact.getAddresses().size()) {
